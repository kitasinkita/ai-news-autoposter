...
[![Version](https://img.shields.io/badge/Version-2.6.1-green.svg)](https://github.com/kitasinkita/ai-news-autoposter/releases)

## 📖 概要

AI News AutoPosterは、**Claude Sonnet 4**と**Gemini 2.5 Flash**を活用して**AIニュース**に関する記事を自動生成・投稿するWordPressプラグインです。**v2.6.1**では設定画面を3タブ構成（共通設定・定型プロンプト設定・フリープロンプト設定）に再構成し、システムの安定性とUnsplash画像生成機能の信頼性を向上させました。フリープロンプトモードと定型プロンプトモードの使い分けが直感的になり、より柔軟な記事生成をサポートします。

## 🆕 v2.6.1の最新機能

### 🎯 設定画面の3タブ構成（新機能）
- **共通設定タブ** - AIモデル、スケジュール、その他の両モード共通設定
- **定型プロンプト設定タブ** - キーワード、記事生成設定、ニュースソース等の定型プロンプト固有設定
- **フリープロンプト設定タブ** - 自由なプロンプト入力専用設定
- **直感的な操作** - 設定項目が論理的に分類され、使いやすさが向上

### 🔧 システム安定性向上
- **Unsplash画像生成の信頼性向上** - 断続的エラーの解決、成功率100%を確認
- **参考情報源表示の最適化** - 単一リスト形式で安定した表示
- **エラーハンドリング改善** - より安定した記事生成プロセス
...
### v2.6.1 (2025-07-16) 🆕
- **設定画面の3タブ構成** - 共通設定・定型プロンプト設定・フリープロンプト設定の論理的分類
- **設定項目の再配置** - AIモデル・スケジュール設定を共通設定に移動、使いやすさ向上
- **Unsplash画像生成の安定性向上** - 断続的エラーの解決、3回連続テスト成功率100%確認
- **参考情報源表示の最適化** - 単一リスト形式で安定した表示を維持
- **システム安定性向上** - エラーハンドリング改善、より信頼性の高い記事生成
- **動作確認完了** - 92-122秒の実行時間で全機能正常動作確認済み
